# == 입력 ==
# 모터 상태(10) + 목표 위치(3) + 손끝 위치(3) + [목표 위치 - 손끝 위치](3) + 목표 각도(3) + 손끝 각도(3) + [목표 각도 - 손끝 각도](3) + 목표 각도(3) + 손끝 각도(3) + [목표 각도 - 손끝 각도](3)
# == 보상 ==
# 손 끝과 목표 지점이 가까울 수록
# 적은 step일 수록
...
class CarArmEnv(MujocoEnv):
    ...
    def step(self, action):
        ...
        # 각도
        cos_theta_1 = np.clip(np.dot(obs[19:22], obs[22:25]), -1.0, 1.0)
        aim_dist = np.arccos(cos_theta_1)

        cos_theta_2 = np.clip(np.dot(obs[28:31], obs[31:34]), -1.0, 1.0)
